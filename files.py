...
class OutputDir(object):
    ...
    def list(self,prefix=None,file_ext=None):
        ...
        if prefix is None and file_ext is None:
            ...
            L = [file for file in self.list() if prefix in file.split('_')[0]]
        elif prefix is None and file_ext is not None:
            L = [file for file in self.list() if (os.path.splitext(file)[1] == file_ext)]
        else: 
            L = [file for file in self.list() if (prefix in file.split('_')[0] and os.path.splitext(file)[1] == file_ext)]
        return sorted(L)
