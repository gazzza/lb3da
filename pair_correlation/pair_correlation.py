...
# def LatticeCoords():
#     sep  = 2
#     lattice_coords = np.array([(x,y,z) for x in range(Lx+1)[slice(0,Lx+1,sep)] for y in range(Ly+1)[slice(0,Ly+1,sep)] for z in range(Lz+1)[slice(0,Lz+1,sep)]])
#     return lattice_coords


def FittingFunc(r,gr,order):
    p_coeff= np.polyfit(r,gr,order)
    p=np.poly1d(p_coeff)
    x=np.linspace(min(r),max(r),1000)
    print 'p: ' + str(p)
    print sp.quad(p,np.min(r),np.max(r))
    return (x,p(x))


def MDListMaker(n,nstart):
    
    flist = os.listdir('.')
    mdlist=[]
    for files in flist:
        pathsplit = os.path.splitext(files)
        splitfile = pathsplit[0].split('_')
        if splitfile[0] == 'md-cfg' and pathsplit[1] == '.asc':
            filenum = int(splitfile[3].split('-')[0].split('t')[1])
            if filenum % n==0 and filenum > nstart:
                mdlist.append(files)
        mdlist.sort()
    return mdlist

def PeriodicFilter(array):
...
def PairCorrelationFunction(filename):
    ...
    allcoords = CoordListFromFile(filename) 

    init_shell = 15
    
    totalN = allcoords.shape[0]
